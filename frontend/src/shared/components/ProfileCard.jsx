    ...
    <div className="max-w-sm mx-auto bg-white rounded-xl shadow-lg overflow-hidden border border-gray-300 mt-10">
      <div className="flex flex-col items-center p-6">
        ...
        <div className="w-full text-left text-sm space-y-1">
          <p><span className="font-semibold">Cédula:</span> {user.cedula || "N/A"}</p>
          ...
          <p><span className="font-semibold">Teléfono:</span> {user.telefono || "Sin teléfono"}</p>
          <p><span className="font-semibold">Correo:</span> {user.email || user.correo || "Sin correo"}</p>
          <p>
            <span className="font-semibold">Estado:</span>{' '}
            <span
              className={((user.estado || '').toLowerCase() === 'activo' ? 'text-green-600 font-bold' : 'text-red-600 font-bold')}
            >
