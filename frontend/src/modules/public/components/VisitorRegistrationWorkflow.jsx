...
const VisitorRegistrationWorkflow = ({
  presentation = 'page',
  onBusyChange,
  onDirtyChange,
  onRegistered,
  ...
  const isBusy = submitting || documentLoading || cameraChecking || cameraCapturing;
  const hasFormValues = Boolean(
    formSnapshot
    && Object.entries(formSnapshot).some(([key, value]) => key !== 'rol' && String(value || '').trim())
  );
  const isDirty = !successPayload && Boolean(documentImage || hasFormValues);
  ...
  useEffect(() => {
    if (!showFaceCaptureModal && !cameraOpen) return undefined;

    const handleNestedEscape = (event) => {
  ...
  }, [cameraOpen, handleCloseCamera, showFaceCaptureModal]);
  ...
  return (
    <>
      <section className={isModal ? '' : 'min-h-screen bg-[#f8fafc] px-4 py-8 sm:py-12'}>
      <div className={`mx-auto max-w-5xl ${isModal ? 'space-y-6' : 'space-y-10'}`}>
        {!isModal && (
              ...
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
                ...
                {onCloseAfterSuccess && (
                  <button
                    ...
                    className="mt-4 inline-flex w-full items-center justify-center rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-800"
                  >
                    Cerrar y volver al escaner
                  </button>
                )}
              </div>
            )}
      ...
      {showFaceCaptureModal && createPortal(
        <div
          className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 px-4"
          role="dialog"
          aria-modal="true"
          aria-label="Captura facial del visitante"
      ...
      {cameraOpen && createPortal(
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Captura del documento de identidad"
        >
...
export default VisitorRegistrationWorkflow;
