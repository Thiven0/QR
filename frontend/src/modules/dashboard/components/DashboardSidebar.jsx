...
const DashboardSidebar = () => {
  ...
  const baseItems = [
    ...
    },
    {
      ...
      icon: (
        <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.6} stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" d="M3 13.125A4.125 4.125 0 0 1 7.125 9h9.75A4.125 4.125 0 0 1 21 13.125v3.75A2.625 2.625 0 0 1 18.375 19.5H5.625A2.625 2.625 0 0 1 3 16.875v-3.75Z" />
          <path strokeLinecap="round" strokeLinejoin="round" d="M10 19.5V21m4-1.5V21M4.5 9h.375a1.125 1.125 0 0 0 1.125-1.125V7.875A2.625 2.625 0 0 1 8.625 5.25h6.75A2.625 2.625 0 0 1 18 7.875v.198A1.125 1.125 0 0 0 19.125 9H19.5" />
        </svg>
      ),
        ...
        <div>
          ...
          <nav className="mt-3 flex flex-col gap-1">
            {visibleItems.map(({ to, label, icon, isActiveOverride }) => (
              <NavLink key={to} to={to} className={({ isActive }) => linkClasses(isActiveOverride ?? isActive)}>
                {icon}
                <span>{label}</span>
              </NavLink>
            ))}
